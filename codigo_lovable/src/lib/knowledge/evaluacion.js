...
export function empujeAxialDelPunto(porcentajeBep) {
  ...
  return REGIMENES_EMPUJE.find((r) => porcentajeBep >= r.desde && porcentajeBep < r.hasta);
}
...
export const CONSIDERACIONES = {
  ...
  caudalCercaRunout: {
    ...
    causas: ["Poca contrapresión en el cabezal", "Frecuencia alta con línea poco restringida"],
    acciones: [
  ...
  caudalJustoSobreMcsf: {
    ...
    significado: "El punto es admisible pero tiene poco margen antes de entrar en recirculación.",
  ...
  fueraVentanaBep: {
    ...
    estado: "limite",
    mensaje: "Operando al X % del BEP; fuera de la ventana recomendada de 70–120 %.",
    significado:
  ...
  presionSuccionBaja: {
    ...
    estado: "fuera",
    mensaje: "La presión de succión está por debajo del mínimo: riesgo de cavitación.",
    significado:
  ...
  potenciaSobreNominal: {
    ...
    estado: "fuera",
    mensaje: "La potencia que pide la bomba está en el límite de lo que el motor puede entregar.",
    significado:
  ...
  presionDescargaSobreVrp: {
    ...
    condicion: "presión de descarga > 95 % del seteo de la VRP o de la presión máxima de trabajo",
    estado: "fuera",
    mensaje: "La presión de descarga está en el límite de lo admisible para esta unidad.",
    significado:
  ...
  frecuenciaNoCoincide: {
    ...
    estado: "ninguno",
    mensaje: "El punto medido cae sobre la curva de X Hz, no sobre la de Y Hz.",
    significado:
 ...
 */
/**
 * Daños que sólo tienen sentido en un régimen de empuje concreto. Varias
 * consideraciones cubren los dos extremos de la curva a la vez (por ejemplo,
 * un head bajo puede venir de runout o de desgaste a caudal bajo), así que
 * listan ambos daños. Cuando el punto ya dice en qué régimen está, el daño del
 * régimen contrario es imposible y no debe reportarse.
 */
const DANOS_DIRECCIONALES = { "DA-01": "upthrust", "DA-02": "downthrust" };
...
export function danoEsperado(resultado) {
  ...
  // En transición, o sin caudal respecto al BEP, no se descarta ninguno.
  const direccion =
    regimen && (regimen.tipo === "upthrust" || regimen.tipo === "downthrust") ? regimen.tipo : null;
  const compatible = (id) =>
    !direccion || !DANOS_DIRECCIONALES[id] || DANOS_DIRECCIONALES[id] === direccion;
...
export function resumenParaPrompt({ incluirPendientes = true } = {}) {
  ...
  for (const c of Object.values(CONSIDERACIONES)) {
    ...
    if (c.danos?.length)
      l.push(`  Daños posibles: ${c.danos.map((d) => DANOS[d]?.titulo ?? d).join("; ")}.`);
