# PROMPT PARA LOVABLE — HPS-DOOM · VARIANTE A (chat IA fijo en columna derecha)

> Pega todo este documento como primer mensaje en un proyecto nuevo de Lovable.
> Misma app que la Variante B, pero el chat de IA es una **columna persistente a la derecha**,
> siempre visible, que cambia de contexto según la pestaña activa.
...
## 5. LAYOUT GLOBAL — TRES COLUMNAS

Esta es la diferencia clave de esta variante. La pantalla se divide en **tres zonas fijas**,
de izquierda a derecha, sin scroll de página:

```
┌────────────┬───────────────────────────────────┬──────────────────┐
│            │  HEADER: HPS-DOOM   [bomba] [☀/☾] │                  │
│  SIDEBAR   ├───────────────────────────────────┤   CHAT IA        │
│  (200px)   │                                   │   (columna fija) │
│            │      CONTENIDO DE LA PESTAÑA      │   ~380px         │
│  4 items   │                                   │   siempre visible│
│            │                                   │                  │
└────────────┴───────────────────────────────────┴──────────────────┘
```
...
### 5.2 Columna central de contenido

- Ocupa el espacio entre el sidebar y el chat, con padding de 20px.
- Header propio: título **HPS-DOOM** centrado, en mayúsculas, peso medio. A la derecha del header:
  chip con la bomba seleccionada + toggle de tema.
- El contenido cambia según la pestaña. Navegación client-side (react-router) con rutas
  `/curvas`, `/arranques`, `/documentacion`, `/creditos`. Ruta por defecto: `/curvas`.

### 5.3 Columna derecha — CHAT IA persistente

- Ancho fijo de 380px (mínimo 320px), altura completa, borde hairline a la izquierda,
  superficie de panel ligeramente distinta al plano de página para que se lea como un riel aparte.
- **Está presente en las cuatro pestañas y nunca se desmonta al navegar.**
- Botón de colapsar (chevron derecho) en su cabecera: al colapsar se reduce a una barra de 44px con
  el icono de chat en vertical, y la columna central se expande. Animación de 200 ms.
- **El chat es contextual:** su cabecera muestra un chip con el contexto activo
  (`CURVAS` / `ARRANQUES` / `GENERAL`), y mantiene **hilos separados por contexto**. Al cambiar de
  pestaña, el riel muestra el hilo correspondiente sin perder los otros.
- En `Documentación` y `Créditos` el contexto es `GENERAL` y el chat sigue disponible como consulta libre.

---

## 6. MENÚ RETRÁCTIL DE BOMBAS (componente compartido)

- Panel angosto anclado arriba a la izquierda de la columna central, en las pestañas
  Curvas de eficiencia y Arranques.
...
- La selección vive en `PumpContext` y es **global**: cambiar de bomba en una pestaña la cambia en todas,
  y actualiza el chip de contexto del riel de chat.
- Cambiar de bomba recarga los datos y las curvas, y **limpia el resultado de evaluación**, pero
  no borra el historial del chat.

---

## 7. PESTAÑA "CURVAS DE EFICIENCIA" (columna central)

Como el chat vive en el riel derecho, la columna central se reorganiza así:
...
│                                                              │
├──────────────────────────────────────────────────────────────┤
│  INPUT DE OPERACIÓN ACTUAL  (fila horizontal, 4 campos)      │
└──────────────────────────────────────────────────────────────┘
```

El gráfico gana todo el espacio que antes ocupaba el chat, y el recuadro de input de operación
pasa a ser una **franja horizontal** debajo del gráfico.
...
### 7.3 Franja INPUT DE OPERACIÓN ACTUAL

Cuatro campos numéricos **en una sola fila horizontal**, cada uno con su etiqueta encima:
...
Al final de la fila, un bloque de acción con: el indicador **¿DENTRO DE LA CURVA?**, el botón
primario **"EVALUAR"** y el secundario **"LIMPIAR"**.

- Estado inicial del indicador: `— SIN DATOS`, en texto atenuado, sin color.
...
- "EVALUAR" está deshabilitado hasta que los 4 campos tengan valor.
- "LIMPIAR" vacía los campos, borra el marcador del gráfico y devuelve el indicador a `SIN DATOS`.
...
### 7.4 Enganche con el riel de chat

- Al evaluar correctamente, el riel derecho (contexto `CURVAS`) habilita un botón primario
  **"INICIAR ANÁLISIS"** en la parte superior de su área de mensajes.
- Mientras no se haya evaluado, ese botón está deshabilitado y debajo se lee, en una línea de texto
  atenuado: `Ingresa los datos de operación para habilitar el análisis.`
- Al pulsarlo, el asistente responde (mock, con escritura progresiva) con tres bloques cortos,
  con estos encabezados literales:
  - `RECOMENDACIONES INICIALES`
  - `POSIBLES DAÑOS`
  - `ESTADO DE OPERACIÓN`
- Desde ahí, la conversación sigue libre en el mismo riel.

---

## 8. PESTAÑA "ARRANQUES" (columna central)

```
┌──────────────────────────────┬───────────────────────────────┐
│  DATOS DE LOCACIÓN           │  DATOS DE BOMBA               │
│  3 campos + botón            │  [menú retráctil] + 2 columnas│
├──────────────────────────────┴───────────────────────────────┤
│  RESUMEN DE ARRANQUE                                          │
│  (tarjetas compactas de solo lectura)                         │
└───────────────────────────────────────────────────────────────┘
```
...
deshabilitado hasta que los tres campos tengan valor y haya una bomba seleccionada. Al pulsarlo,
la respuesta aparece en el **riel de chat derecho** (contexto `ARRANQUES`), no en esta columna.
...
### 8.3 Recuadro RESUMEN DE ARRANQUE

Fila de 3 o 4 tarjetas compactas de solo lectura, cada una con una etiqueta en mayúsculas 11px y
un valor grande con tabular-nums. Contenido:

- `MARGEN VS. PRESIÓN A ROMPER` — valor mock en `psi` + indicador de estado (icono + color + texto).
- `CAUDAL ESPERADO VS. NOMINAL` — porcentaje mock + indicador de estado.
- `MOTOR MÍNIMO REQUERIDO` — `800 hp / 597 kW` (de la bomba seleccionada).
- `FRECUENCIA DE ARRANQUE SUGERIDA` — valor mock en `Hz`.

Antes de generar el análisis, las cuatro tarjetas muestran `—` en texto atenuado.
Los valores son mock: **no calcules nada real**, deja `// TODO(backend): calcular en el servicio.`

### 8.4 Enganche con el riel de chat

- Al pulsar "GENERAR ANÁLISIS DE ARRANQUE", el riel (contexto `ARRANQUES`) responde con
  tres bloques cortos, con estos encabezados literales:
  - `LIMITACIONES`
  - `RECOMENDACIONES`
  - `OTROS`
...
Diseño libre pero sobrio y consistente con el resto. Vive en la columna central; el riel de chat
sigue visible en contexto `GENERAL`. Incluye:
...
- Una grilla de tarjetas de documento (2 columnas, porque el riel ocupa ancho). Cada tarjeta:
...
Diseño libre, una sola columna centrada dentro de la columna central. Incluye:
...
## 11. COMPONENTE `<ChatRail />` (el riel derecho)

Un único componente montado en el layout raíz, **fuera del router**, para que no se desmonte al navegar.

**Estructura vertical:**

1. **Cabecera compacta:** título `CHAT IA`, chip de contexto (`CURVAS` / `ARRANQUES` / `GENERAL`),
   chip con la bomba activa, botón **"Nueva conversación"** (papelera) que limpia el hilo de ese
   contexto, y botón de colapsar el riel (chevron).
2. **Zona de acción contextual:** cuando la pestaña activa es Curvas o Arranques, aquí aparece el
   botón primario correspondiente (`INICIAR ANÁLISIS` / `GENERAR ANÁLISIS DE ARRANQUE`), con su
   estado deshabilitado y su línea de texto atenuado. En contexto `GENERAL` esta zona no se renderiza.
3. **Área de mensajes** con scroll propio y autoscroll al último mensaje.
...
4. **Barra de input al pie**, siempre visible: textarea de una línea que crece hasta 3 líneas,
   placeholder `Escribe tu consulta…`, y botón de enviar en forma de **triángulo ▷**.
   `Enter` envía, `Shift+Enter` salta línea.

**Comportamiento:**

- El historial se guarda en `SessionContext`, **separado por contexto** (`curvas`, `arranques`, `general`).
  Cambiar de pestaña cambia el hilo visible; los otros hilos se conservan intactos.
- **El historial se borra al terminar la sesión**: vive solo en memoria, no se persiste en
  `localStorage`, `sessionStorage`, cookies ni ningún almacenamiento. Al recargar la página los
  hilos aparecen vacíos. Esto es un requisito, no una sugerencia.
...
- El servicio debe recibir un objeto de contexto tipado que ya incluya: contexto activo, bomba
  seleccionada, inputs actuales (operación o locación) y el historial del hilo. Aunque hoy los
  ignore, la firma debe estar lista.
...
- **Responsive:** optimizado para 1440px+. Entre 1024 y 1440px el riel se colapsa por defecto.
  Por debajo de 1024px el riel se convierte en un panel deslizante sobre el contenido, abierto por
  un botón flotante, y el sidebar pasa a barra superior con iconos. No inviertas esfuerzo en móvil.
...
2. El riel de chat es visible en las 4 pestañas, no se desmonta al navegar y se puede colapsar.
3. El riel mantiene hilos separados por contexto y el chip de contexto cambia con la pestaña.
4. El toggle claro/oscuro cambia toda la app, gráfico incluido, sin colores rotos.
5. El menú retráctil colapsa y expande, y la bomba seleccionada se comparte entre pestañas.
6. Cambiar de bomba actualiza los 13 datos y redibuja las curvas del gráfico.
7. Con los 4 campos de operación llenos, "EVALUAR" dibuja el marcador naranja en el gráfico y
   el indicador muestra icono + color + texto.
8. "INICIAR ANÁLISIS" solo se habilita después de evaluar, y produce los tres bloques indicados.
9. En Arranques, "GENERAR ANÁLISIS DE ARRANQUE" llena las tarjetas de resumen y produce
   LIMITACIONES / RECOMENDACIONES / OTROS en el riel.
10. Al recargar la página, todos los hilos aparecen vacíos.
11. No hay ni una fórmula, ni un párrafo explicativo, ni texto de relleno en la interfaz.
12. Ningún componente lee datos directamente de un archivo mock: todo pasa por `services/`.
...
- No montes el riel de chat dentro de cada página: debe vivir en el layout raíz.
